...
print("Немного погодя вы вспоминаете что вчера было 31 декабря и вы явно засыпали в другой комнате.. ")
q = input("(нажми Enter)")
...
q = input("(нажми Enter)")
print()
print("Чтобы использовать предмет из своего инвентаря, напишите используй 'название предмета', например команда используй молоток сделает действие с этим предметом.")
...
q = input("(нажми Enter)")
print()
print("Чтобы подобрать предмет в свой инвентарь, напишите подбери 'название предмета', например, подбери отвертку (в нужном падеже)")
q = input("(нажми Enter)")
print()
...
vidno = 0
inventory = set()
seif = False  # переменная показывает, открыт ли сейф
kran = False  # переменная показвает, починен ли кран
kamin = True  # переменная показывает, горит или не горит камин
gaech = "и гаечный ключ"  # переменная показывает, лежит или не лежит гаечный ключ под ковром
vedro = "вы нашли пустое ведро" # переменная показывает, лежит пустое ведро в шкафу, или нет
otvertk = "вы сразу заметили какой-то предмет, торчавший из-под угольков," #  переменная показвает, лежит отвертка в камине, или нет
end = False
kluch = " вы нашли ключ."
while True:
    # часть кода, отвечающая за команды прямо, вниз, вверх, направо, налево
    if vvod == "вверх" or vvod == "наверх":
        ...
        print("Посмотрев налево, вы увидели камин.")
        vidno = "налево"
    # часть кода, отвечающая за исследование предметов
        ...
        print("Под ковром вы увидели сейф, полностью вделанный в пол", gaech)
        ...
        print("Поискав в шкафчике,", vedro)
    elif vvod == "камин" and vidno == "налево":
        print("Подойдя ближе к камину,", otvertk, "а также письмо, лежащее рядом с камином")
        ...
        if vvod == "да":
            ...
            if parol == "2023":
                ...
                time.sleep(1)
                print("Сейф открылся и, пошарив рукой,", kluch)
                seif = True
            else:
                print("Пароль неверен!")
    # часть кода, отвечающая за подбор предметов
    ...
    # часть кода, отвечающая за использование предметов
        ...
        if vidno == "налево":
            if "ведро с водой" in inventory:
                ...
                kamin = False
                otvertk = "вы увидели мокрые угли, из которых торчал какой-то предмет,"
            else:
        ...
        if vidno == "прямо":
            if "ключ" in inventory:
                ...
                time.sleep(1)
                print("На этом игра заканчивается.")
                end = True
            else:
                print("У вас нету ключа!")
        else:
            print("Команда не сработала из-за того вы смотрите не туда")
    # часть кода, отвечающая за разное
    elif vvod == "помощь":
        print("Чтобы осмотреться, напишите команды вверх, вниз, прямо, влево, вправо")
        print("Чтобы исследовать предмет, напишите название предмета, которого хотите исследовать, например команда 'дверь' скажет информацию о двери")
        print("(все исследуемые предметы пишите одним словом, например не надо писать ржавая труба, правильно написать труба)")
        ...
        print("Возможно вы пытаетесь подобрать предмет, который вовсе не нужен, который неправильно назвали или уже подобрали.")
    else:
